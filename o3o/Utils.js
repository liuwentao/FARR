...
// replaces all occurences of o object in a string
...
// repeats string l times and returns it
String.prototype.repeat = function (l) {
	return ((l > -1) ? (new Array(l + 1).join(this)) : this);
};
...
Array.prototype.includeAll = function (ary) {
	...
	if (!this.include(ary[i])) return false;
	return true;
}
...
(function () {
	...
	var cx = /[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
		...
		gap, indent, meta = { // table of character substitutions'\b': '\\b',
			'\t': '\\t',
			'\n': '\\n',
			'\f': '\\f',
			'\r': '\\r',
...
(function () {
	...
	function receiveKeys() {
		FARR.setStrValue("receivekeys", "1");
	}

	function setStatusBarMenu(txt) {
		FARR.setStrValue("addmenu.statusbar", txt);
	}

	function setContextMenu(txt) {
		FARR.setStrValue("addmenu.contextmenu", txt);
	}
	...
	function setSearch(txt) {
		FARR.setStrValue("setsearch", txt);
	}

	function setSearchNoGo(txt) {
		FARR.setStrValue("setsearchnogo", txt);
	}
	...
	function hideWindow() {
		FARR.setStrValue("window.hide");
	}

	function showWindow() {
		FARR.setStrValue("window.show");
	}

	function toggleWindow() {
		FARR.setStrValue("window.toggle");
	}

	function setRichEditMode(txt) {
		receiveKeys();
		FARR.setStrValue("window.richeditmode", txt);
	}
	...
	function setCliboard(txt) {
		FARR.setStrValue("clipboard", txt);
	}
	...
	// ---------------- FARR wrappers ----------------
	if (typeof (Utils.doRequest) !== 'function') {
		Utils.doRequest = function doRequest(url, r) {
	...
	if (typeof (Utils.alert) !== 'function') {
		Utils.alert = function alert(txt) {
			FARR.debug(txt);
		}
	}
	if (typeof (Utils.error) !== 'function') {
		Utils.error = displayAlertMessage;
	}
